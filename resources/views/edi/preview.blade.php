@extends('layouts.app')
@section('title', 'Aperçu · EDI Converter')
...
@push('styles')
    <style>
        ...
        .btn-download:hover {
            background: #00f5de;
            box-shadow: 0 0 30px var(--cyan-glow), 0 4px 16px rgba(0, 0, 0, .3);
            transform: translateY(-2px);
        }
...
@section('content')
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            ...
            <div class="stat-value">{{ $previewRows->pluck('data')->pluck('transport_mode')->filter()->unique()->count() }}
            </div>
        ...
        <div class="table-footer">
            ...
            @if ($totalCount > 50)
                <span class="truncate-notice">
                    ...
                    Tous les {{ $totalCount }} enregistrements sont affichés ci-dessous.
    ...
    <div class="download-cta">
        <div class="cta-text">
            ...
            <p>Le fichier IFTMIN contiendra les {{ $totalCount }} enregistrements, formatés avec en-têtes, filtres et
                mise en page professionnelle.</p>
            <div class="timer">
...
@section('scripts')
    <script>
        ...
        // Afficher la barre "afficher plus" si total > 50
        if ({{ $totalCount }} > 50) {
            showMoreBar.style.display = 'block';
        }
        ...
        searchInput.addEventListener('input', function() {
            ...
            rowCount.textContent = visible + ' ligne' + (visible > 1 ? 's' : '') + ' affichée' + (visible > 1 ?
                's' : '');
            showMoreBar.style.display = 'none';
        });

        // Feedback boutons de téléchargement
        document.querySelectorAll('#dlBtnTop, #dlBtnBottom').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                this.innerHTML =
                    `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="animation:spin 1s linear infinite"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg> Génération…`;
                setTimeout(() => {
